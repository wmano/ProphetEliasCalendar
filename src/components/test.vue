      ...
      >
        ...
        <transition name="fade" mode="out-in">
          <div v-if="isCalendarExpanded" :key="currentMonth + currentYear">
            ...
            <!-- Legend -->
            <div class="grid grid-cols-3 gap-4 text-sm text-purple-200 mt-4">
...
const selectDate = (date) => {
  ...
  bibleText.value = null // Clear previous Bible text when selecting a new date
}
...
const getMarkerColor = (fastenInformation) => {
  if (fastenInformation.includes('Streng')) return 'red'
  if (fastenInformation.includes('Normal')) return 'blue'
  if (fastenInformation.includes('Leicht')) return 'yellow'
  return null
}
